...
$files = array(
	'Nginx'=>['name'=>'Nginx Logs', 'path'=> LOG_PATH.'nginx/error.log'],
	'FPM'=>['name'=>'Php5-Fpm Logs', 'path'=> LOG_PATH.'upstart/php5-fpm.log'],
);
// Set a Smart default: Apache Error log
$log =(!isset($_GET['p'])) ? 'Nginx': $_GET['p'];
...
<div id="layout">
	...
	foreach($files as $k=>$f){
		if(!is_file($f['path'])){
			echo 'File ' . $f['path'] . ' does not exists!'; // File does not exist, remove it from the array, so it does not appear in the menu.
			unset($files[$k]);
			continue;
		}
